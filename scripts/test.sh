...
time ./simfactory/bin/sim --machine="actions-$ACCELERATOR-$REAL_PRECISION" create-run TestJob01_temp_1 --cores 1 --num-threads 2 --testsuite --select-tests=CarpetXUtils
ONEPROC_DIR="$(./simfactory/bin/sim --machine="actions-$ACCELERATOR-$REAL_PRECISION" get-output-dir TestJob01_temp_1)/TEST/sim"

time ./simfactory/bin/sim --machine="actions-$ACCELERATOR-$REAL_PRECISION" create-run TestJob01_temp_2 --cores 2 --num-threads 1 --testsuite --select-tests=CarpetXUtils
...
# cd "$PAGESSPACE"
# python3 "$CARPETXUTILSSPACE/scripts/store.py" "$WORKSPACE/Cactus/repos/CarpetXUtils" "$ONEPROC_DIR" "$TWOPROC_DIR"
# python3 "$CARPETXUTILSSPACE/scripts/logpage.py" "$WORKSPACE/Cactus/repos/CarpetXUtils"
# 
# # Store HTML results
# git add docs
# git add records
